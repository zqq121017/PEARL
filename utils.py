...
def weighted_pearson_correlation(x1, x2=None):
    ...
    weight = torch.linspace(0.45, 1.55, num_features).to(x1.device)
    ...
    corr = torch.mm(x1_centered, x2_centered.t()) / (torch.norm(x1_centered, dim=1).unsqueeze(1) * torch.norm(x2_centered, dim=1).unsqueeze(0))
